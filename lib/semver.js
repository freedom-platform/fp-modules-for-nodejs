...
exports.latestForRevision = function(versions, revision) {
    if(!versions || versions.length==0) {
        return false;
    }
    
    if (revision) {
        revision = fillVersionZeros(""+revision);
	...
	if (revisionIndex === versions.length-1)
		return false;
	...
	for (var i = revisionIndex+1 ; i< versions.length ; i++)
	{
		...
		if (version[0] != revision[0]) return lastMatchingVersion;
	}
